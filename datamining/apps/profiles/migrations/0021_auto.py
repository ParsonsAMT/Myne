...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding M2M table for field feeds on 'Person'
        db.create_table('profiles_person_feeds', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('person', models.ForeignKey(orm['profiles.person'], null=False)),
            ('feed', models.ForeignKey(orm['feedjack.feed'], null=False))
        ))
        db.create_unique('profiles_person_feeds', ['person_id', 'feed_id'])

        # Adding M2M table for field feeds on 'Section'
        db.create_table('profiles_section_feeds', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('section', models.ForeignKey(orm['profiles.section'], null=False)),
            ('feed', models.ForeignKey(orm['feedjack.feed'], null=False))
        ))
        db.create_unique('profiles_section_feeds', ['section_id', 'feed_id'])
    
    
    def backwards(self, orm):
        
        # Removing M2M table for field feeds on 'Person'
        db.delete_table('profiles_person_feeds')

        # Removing M2M table for field feeds on 'Section'
        db.delete_table('profiles_section_feeds')
