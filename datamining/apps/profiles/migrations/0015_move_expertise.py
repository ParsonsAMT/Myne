...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            #dumping faculty and student expertise into the same table
            for fm in orm.FacultyMember.objects.all():
                for e in fm.expertise.all():
                    fm.temp_expertise.add(e)
                fm.save()
    
            for s in orm.Student.objects.all():
                for e in s.expertise.all():
                    s.temp_expertise.add(e)
                s.save()
    ...
    def backwards(self, orm):
        if not db.dry_run:
            ...
            for p in orm.Person.objects.all():
                for e in p.temp_expertise.all():
                    try:
                        p.facultymember.expertise.add(e)
                    except:
                        try:
                            p.student.expertise.add(e)
                        except:
                            print "no specific person found for %s" % (p)
                p.save()
    ...
    models = {
        ...
        'profiles.person': {
            ...
            'temp_expertise': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': "orm['profiles.Expertise']", 'null': 'True', 'blank': 'True'}),
            'updated_at': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
