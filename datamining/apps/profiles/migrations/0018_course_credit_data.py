...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        if not db.dry_run:
            #moving credits to minimum credits within course
            for course in orm.Course.objects.all():
                course.minimum_credits = course.credits
    ...
    def backwards(self, orm):
        if not db.dry_run:
            #moving minimum credits to credits within course
            for course in orm.Course.objects.all():
                course.credits = course.minimum_credits
                if course.created_at is None:
                    course.created_at = datetime.datetime.now()
                course.save()
